import { buildOmdbUrl, fetchMovieDetails, searchMovies } from '@/lib/omdb';
...
describe('buildOmdbUrl', () => {
  ...
  it('appends type to a search URL when provided', () => {
    const url = buildOmdbUrl({ s: 'batman', page: 2, type: 'series' });
    expect(url).toBe('http://www.omdbapi.com/?apikey=b8186782&s=batman&page=2&type=series');
  });

  it('omits type from a search URL when not provided', () => {
    const url = buildOmdbUrl({ s: 'batman', page: 2 });
    expect(url).not.toContain('type=');
  });

  it('appends y to a search URL when provided', () => {
    const url = buildOmdbUrl({ s: 'batman', page: 1, year: 2008 });
    expect(url).toBe('http://www.omdbapi.com/?apikey=b8186782&s=batman&page=1&y=2008');
  });

  it('omits y from a search URL when not provided', () => {
    const url = buildOmdbUrl({ s: 'batman', page: 1 });
    expect(url).not.toContain('&y=');
  });

  it('includes both type and y when both are provided', () => {
    const url = buildOmdbUrl({ s: 'batman', page: 1, type: 'movie', year: 2008 });
    expect(url).toContain('type=movie');
    expect(url).toContain('y=2008');
  });
...
describe('searchMovies', () => {
  it('returns the parsed MovieSearchResponse on success', async () => {
    ...
    const result = await searchMovies({ searchTerm: 'list', page: 1 });
  ...
  it('forwards the type filter to the request URL', async () => {
    vi.stubGlobal('fetch', vi.fn().mockResolvedValue(jsonResponse(searchSuccessBody)));
    await searchMovies({ searchTerm: 'batman', page: 1, type: 'series' });
    const calledUrl = String((fetch as ReturnType<typeof vi.fn>).mock.calls[0][0]);
    expect(calledUrl).toContain('type=series');
  });

  it('forwards the year filter to the request URL', async () => {
    vi.stubGlobal('fetch', vi.fn().mockResolvedValue(jsonResponse(searchSuccessBody)));
    await searchMovies({ searchTerm: 'batman', page: 1, year: 2008 });
    const calledUrl = String((fetch as ReturnType<typeof vi.fn>).mock.calls[0][0]);
    expect(calledUrl).toContain('y=2008');
  });

  it('throws a transport OmdbError on HTTP 404', async () => {
    vi.stubGlobal(
      'fetch',
      vi.fn().mockResolvedValue(jsonResponse({ Response: 'False', Error: 'Not found' }, { status: 404 }))
    );
    await expect(searchMovies({ searchTerm: 'list' })).rejects.toMatchObject({
      kind: 'transport',
      status: 404
    });
  });
  ...
  it('throws a transport OmdbError when fetch rejects', async () => {
    ...
    await expect(searchMovies({ searchTerm: 'list' })).rejects.toMatchObject({ kind: 'transport' });
  });
  ...
  it('throws an api OmdbError with "Too many results." for that payload', async () => {
    ...
    await expect(searchMovies({ searchTerm: 'l' })).rejects.toMatchObject({
      kind: 'api',
  ...
  it('throws an api OmdbError with "Movie not found!" for that payload', async () => {
    ...
    await expect(searchMovies({ searchTerm: 'zzzznotreal' })).rejects.toMatchObject({
      kind: 'api',
